...
function deliver_antrea {
    ...
    (cd $GIT_CHECKOUT_DIR && make policy-recommendation && make clickhouse-monitor && make clickhouse-server && make theia-manager && make anomaly-detection && make spark-jobs)
    docker save -o theia-spark-jobs.tar projects.registry.vmware.com/antrea/theia-saprk-jobs:latest
