...
def test_get_flow_type(test_input, expected_flow_type):
    ...
    flow_type = pr.get_flow_type(destinationServicePortName,
...
def test_recommend_policies_for_ns_allow_list(test_input, expected_policies):
    recommend_polices_yamls = pr.recommend_policies_for_ns_allow_list(
        test_input)
...
def test_recommend_k8s_policies(spark_session, flows_input, expected_policies):
    test_df = spark_session.createDataFrame(
        flows_input, pr.FLOW_TABLE_COLUMNS + ["flowType"]
    )
    recommend_k8s_polices_yamls = pr.recommend_k8s_policies(test_df)
    recommend_k8s_polices_dicts = [
        yaml.load(i, Loader=yaml.FullLoader)
        for i in recommend_k8s_polices_yamls
    ]
...
def test_recommend_antrea_policies(
    ...
    recommend_polices_yamls = pr.recommend_antrea_policies(
