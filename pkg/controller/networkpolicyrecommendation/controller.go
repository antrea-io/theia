...
func (c *NPRecommendationController) startSparkApplication(npReco *crdv1alpha1.NetworkPolicyRecommendation) error {
	...
	err = commands.ParseRecommendationName(npReco.Name)
	if err != nil {
		return IlleagelArguementError{fmt.Errorf("invalid request: Job name is not a valid policy recommendation job name")}
	}
